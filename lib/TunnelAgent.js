...
// Define a fixed port range for the tunnel server
...
class TunnelAgent extends Agent {
    constructor(options = {}) {
        ...
        this.minPort = options.minPort || MIN_PORT;
    ...
    // Helper method to find an available port in the specified range
    async _findAvailablePort() {
        return new Promise((resolve, reject) => {
            const tryPort = (port) => {
                if (port > this.maxPort) {
                    reject(new Error(`No available ports in range ${this.minPort}-${this.maxPort}`));
                    return;
                }
            ...
            // Start with a random port in the range to avoid always using the same port
            const startPort = Math.floor(Math.random() * (this.maxPort - this.minPort + 1)) + this.minPort;
            tryPort(startPort);
        });
    }
    ...
    async listen() {
        ...
        try {
            ...
            this.debug('failed to find available port: %s', error.message);
            throw error;
        }
    }
